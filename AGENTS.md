...
The `overleaf_paper/` directory is a separately cloned Overleaf Git repository for the user's paper writing. Treat it as a read-only reference by default. Do not modify, format, compile, pull, rebase, merge, commit, push, or generate files inside `overleaf_paper/` unless the user gives explicit approval for that specific operation. When the user asks for changes to the paper, report the proposed edits in the conversation for human review first; apply them only after the user explicitly approves the exact scope.
