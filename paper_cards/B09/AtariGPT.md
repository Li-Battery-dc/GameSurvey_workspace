...
- Atari-GPT evaluates multimodal LLMs as low-level control policies on seven Atari games from the Arcade Learning Environment rather than as high-level planners. The models receive the current frame together with a short recent context buffer and must emit structured game actions, after which their cumulative reward over 1,000 timesteps is compared against human, random, and DQN references. The paper also probes visual understanding, spatial reasoning, strategy, and environment identification on eight environments, adding Basic Math to the gameplay set for diagnosis. For this survey, Atari-GPT is best used as an early low-level visual-control contrast case: it is narrower and more system-constrained than later visual-agent suites, but it directly asks whether frontier multimodal models can operate as Atari policies at all.
...
- Perception burden removed: the interface is simplified to discrete action selection with resized frames, fixed frame skipping, and a short rollout horizon rather than full human controller dexterity
...
- Agent scaffold allowed: short frame-and-response history / chain-of-thought prompt
- Is there privileged API access? no
- How close is the setup to human play? low-medium; the models see raw frames, but interaction is mediated through structured action generation, frame resizing, and aggressive frame skipping
- Main ecological-validity trade-off: the benchmark preserves low-level visual control, but it uses a short temporal window, discrete prompted actions, and a 1,000-step horizon that make it more diagnostic than ecological

## 6. Evaluation protocol
- Main score: cumulative reward over 1,000 timesteps, reported in raw form and as human-normalized performance
...
- Reliability or comparability concerns: the short evaluation horizon and restricted temporal context may under-measure long-horizon competence while still being strongly affected by latency, API rate limits, and output-format instability
...
- Best use in Section 1 (taxonomy and evolutionary levels): A useful transition from classical RL game environments to multimodal LLM evaluation. A narrow but concrete example of low-level policy benchmarking rather than a full visual-agent benchmark.
...
- Closest predecessor(s): SmartPlay, LLMPlayStarCraftII, Atari RL baselines in ALE
- Closest follow-up(s): TextAtari, VMage, VideoGameBench
- Best comparison targets inside our corpus: TextAtari, VMage, VideoGameBench, VLMPlayStarCraftII
...
- The rollout protocol uses 1,000 timesteps, 8-frame skipping, and a context buffer of the two previous frames and responses.
...
- Its strongest survey role is in Section 2.6 and Section 4 as evidence about reactive control and system latency, not as a broad ecological benchmark.

### 11.3 Uncertain or needs re-check
- Re-check the appendix if we later need the exact action mappings and per-environment visual-reasoning rubric.
- Re-check whether temperature tuning or other prompt adjustments materially changed performance in follow-up analyses.

## 12. Follow-up reading plan
- Should we read beyond abstract + intro? why? Already done for this audit; further rereading is only needed if the draft later needs exact per-game failure patterns or rubric details.
- Which section to read next if needed: Experimental setup, Table 1, and the visual/spatial reasoning appendix rubric
- Follow-up question(s): Which claims in our draft should attribute failure to latency, and which should attribute it to spatial reasoning weakness?
