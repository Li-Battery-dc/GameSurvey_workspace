...
- Main ecological-validity trade-off: The agentic tool setup surfaces instruction-following failures clearly, but it is more scaffolded than ordinary human chess play.
...
- Human baseline / AI anchor / self-play / model-vs-model setup: random-agent baseline and engine opponents with fixed ratings
- Automatic verifiability: high
- Calibration method: engine-based difficulty and Elo estimation for stronger models
- Anti-contamination argument: chess's combinatorial richness and engine-based play reduce simple benchmark saturation
- Reliability or comparability concerns: the privileged tools and missing move history change what playing chess means relative to human or raw-board settings
...
- Core empirical takeaway: Most non-reasoning models cannot consistently beat even a random opponent, while reasoning-enhanced models do substantially better.
- Notable model failure mode 1: many losses are caused by instruction-following failures rather than purely bad chess
- Notable model failure mode 2: weaker models stall in long conversations without producing legal moves
- Notable model failure mode 3: strong-looking models still rely heavily on the legal-move scaffold
...
- Best use in Section 4 (interaction paradigm): Helpful case for privileged API tools versus raw play.
- Best use in Section 5 (evaluation protocol): Strong example of combining win/loss, Elo, and process-level error metrics.
...
- It reports Win/Loss, Elo for stronger models, and per-game/per-ply indicators including instruction-following errors.
...
- The privileged tool setup is a feature for measurement clarity, but it also limits ecological claims.
