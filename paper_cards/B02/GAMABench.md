...
- GAMABench is a parameterized game-theory benchmark for LLM decision-making in multi-agent settings. It collects eight classical scenarios, emphasizes N-player, multi-round, and sometimes multi-action variants beyond the standard two-player setup, and rescales game-specific raw scores into a unified 0-100 benchmark score. The paper analyzes GPT-3.5 in depth with ten same-model agents, then reports a 13-model leaderboard while also varying prompts, temperatures, and game parameters. For this survey, GAMABench is best read as a formal diagnostic benchmark whose main "dynamic" contribution is adjustable settings and leakage-mitigation-by-diversity, not richer social interaction or ecological interfaces.
...
- Construction note: designed task-game hybrid based on classical game-theory scenarios
...
- Primary capability target: strategic decision-making in formal multi-agent game-theoretic environments
- Secondary capability target(s): arithmetic reasoning, limited belief modeling, robustness to prompt and temperature changes, cooperation-versus-betrayal incentives, sequential decision-making, and generalization across parameterized settings
- Does it test rule grounding / legal action generation? yes
- Does it test strategic planning under uncertainty? yes
- Does it test social reasoning / deception / cooperation? limited only; it probes cooperation-versus-betrayal incentives without dialogue, negotiation, or explicit deception
...
- Observation channel: text prompts describing rules, current round, historical outcomes, and player-specific information such as valuations, rank, or hit rate; there is no free-form agent-to-agent communication
...
- Main ecological-validity trade-off: it gains tunable N-player strategic interdependence, auto-scoring, and parameter sweeps at the cost of heavily mediated prompt interfaces, no natural communication channel, and same-model self-play rather than ecological play loops
...
- Human baseline / AI anchor / self-play / model-vs-model setup: main experiments use ten agents instantiated from the same LLM; appendix experiments add play against fixed strategies, and the framework is described as human-compatible, but there is no broad human baseline or mixed-model competition in the main leaderboard
...
- Anti-contamination argument: weak-to-moderate; the paper explicitly worries that static classical settings may be in model training data and answers this by varying parameters and generating diverse N-player profiles. That is a diversity-based mitigation, not evidence that the benchmark is contamination-free.
- Reliability or comparability concerns: scores depend on benchmark-specific rescaling, the main setup uses same-model agents rather than mixed-model play, prompt wording can shift some scores sharply, and the fixed 20-round horizon may affect strategic behavior in repeated games

## 7. Main contributions
- Contribution 1: Extends mostly two-player game-theoretic LLM evaluation toward N-player, multi-round, and sometimes multi-action settings across eight classical scenarios.
...
- Notable model failure mode 2: prompt phrasing can materially shift performance in some games, especially Public Goods Game, Diner's Dilemma, and Pirate Game
- Notable model failure mode 3: models often fail to infer betrayal-optimal or incentive-consistent play in Public Goods Game and Diner's Dilemma, defaulting instead to cooperative or conservative behavior
...
- What this paper uniquely adds relative to neighbors: It pushes formal strategic benchmarking toward N-player, multi-round, parameterized scenarios and explicitly frames setting diversity as a way to test generalizability and reduce static-setting leakage risk, without abandoning classical game-theory grounding.
...
- Appendix A defines multi-player as games with more than two players, multi-round as repeated play with maintained action history, and multi-action as settings where every player has more than two actions.
...
- The abstract, introduction, and contribution bullets explicitly motivate dynamic game settings and diverse profiles as a way to assess generalizability while minimizing test set leakage risk from static classical scenarios.
...
- The limitations section states that the main evaluation uses ten agents derived from the same LLM and does not test mixed-LLM competition in the same game.
...
- Re-check Appendix G if we later want to cite the framework's human-compatible setup, since the main leaderboard itself does not provide a human baseline.
...
- Follow-up question(s): Should GAMABench serve mainly as a contrast case on parameterized formal diagnostics and leakage-mitigation-by-diversity, rather than as a Level 3 multi-player or social anchor?
